	...
	$.widget('ui.combobox', {
		...
		*/
		init: function() {
			...
			function displayNormalArrow() {
				...
				$(document).unbind('mouseover', displayNormalArrow);
			};
			...
			this.listElem = this.buildList(this.listElem, false).insertAfter(this.arrowElem).hide();
			...
			.keyup(function(e) {
				if ((e.which == KEY_F4) || (options.autoShow))
					that.showList(e);
			})
            .keydown(function(e) {
            	if (options.autoShow)
		...
		*/
		setData: function(key, value) {
			...
			if (key == 'data' || key == 'listContainerTag' || key == 'listHTML') {
				...
				this.listElem = this.buildList(this.listElem, false).replaceAll(this.listElem);
				this[isVisible ? 'showList' : 'hideList']();
			}
		},

		buildList: function(element, bBuildFull) {
			var that = this;
			var options = this.options;
			var tag = options.listContainerTag;
			if (typeof (element) == "undefined") {
				element = $('<' + tag + ' class = "ui-combobox-list">' + '</' + tag + '>');
				element.builded = false;
			}
			...
			if (bBuildFull) {
				...
				$.each(options.data, function(i, val) {
					...
					element.maxwidth = Math.max(val.length, element.maxwidth || 0);
				});
			}
			return element;
		},
		...
		*/
		showList: function(e) {
			...
			this.listElem = this.buildList(this.listElem, true);
